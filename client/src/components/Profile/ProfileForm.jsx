...
function ProfileForm() {
  ...
  const updateUserInfo = (e) => {
    setUser({
      ...
      // [e.target.name] It is also known as computed
      // property name in es6 so need to call frequent route
    });
    /** whenEver we'll some changes into our input we called
     * this fuction from input field here it'll call the function
     * whatever current value we have in useState like
     * name:"", email:"" i put here ...user,
     * so [e.target.name] is from input name which will upadate
     * the current value hint "" e.target.value is the value of input
     * it's key:value formate it will set it the user Variable
     * eventually it upadate the current value from where we calling */
  };
